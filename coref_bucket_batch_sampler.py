...
class BucketBatchSampler(DataLoader):
    ...
    def prepare_batches(self):
        ...
        for _, elem in self.data_source:
            if len(batch) == 0:
                ...
                per_example_batch_len = self.calc_effective_per_example_batch_len(len(elem.augmented_labels))
            elif (len(batch) + 1) * per_example_batch_len > self.max_total_seq_len:
                batch = self.data_source.pad_batch(batch, len(batch[0].augmented_labels))
    ...
    def prepare_eval_batches(self):
        ...
        for doc_key, elem in self.data_source:
            batch = self.data_source.pad_batch([elem], len(elem.augmented_labels))
